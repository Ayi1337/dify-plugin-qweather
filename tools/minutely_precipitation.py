...
MINUTELY_PRECIPITATION_PATH = "/v7/minutely/5m"


class QWeatherMinutelyPrecipitationTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        url = build_url(base_url, MINUTELY_PRECIPITATION_PATH, query)
        data = get_json(url, timeout_seconds=10)

        code = str(data.get("code", "")).strip()
        if code and code not in ("200", "204"):
            raise RuntimeError(f"QWeather Minutely Precipitation API error: code={code}, response={data}")
