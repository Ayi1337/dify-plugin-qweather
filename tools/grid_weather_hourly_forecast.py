...
ALLOWED_HOURS = {"24h", "72h"}
GRID_WEATHER_HOURLY_PATH_TEMPLATE = "/v7/grid-weather/{hours}"


class QWeatherGridWeatherHourlyForecastTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        path = GRID_WEATHER_HOURLY_PATH_TEMPLATE.format(hours=hours)
        ...
        if code and code not in ("200", "204"):
            raise RuntimeError(f"QWeather Grid Hourly Forecast API error: code={code}, response={data}")
