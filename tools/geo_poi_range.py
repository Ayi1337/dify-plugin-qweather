...
POI_RANGE_SUFFIX = "/poi/range"


class QWeatherGeoPoiRangeTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        radius = str(tool_parameters.get("radius", "")).strip()
        ...
        path = geo_v2_path(base_url, POI_RANGE_SUFFIX)
        ...
        if code and code not in ("200", "204"):
            raise RuntimeError(f"QWeather Geo POI Range API error: code={code}, response={data}")
